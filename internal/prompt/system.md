...
<tool name="cve_lookup">Search the NIST National Vulnerability Database for known CVEs affecting a product. Returns CVEs sorted by CVSS severity with ID, description, score, and references. Use after tech_fingerprint or port_scan to check discovered technologies for known vulnerabilities.</tool>
...
- After identifying technologies with tech_fingerprint or service versions with port_scan, use cve_lookup to check for known vulnerabilities.
